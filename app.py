...
def ensure_tables():
    ...
    # Best-effort: if tickets already existed without a unique key on ticket_id,
    # add one so optional DB-level FKs can work later. Ignore failures (e.g.
    # duplicates or insufficient privileges).
    try:
        lakebase.run_write(
            f"""
            DO $$
    ...
    # Prefer a real FK when tickets.ticket_id is unique; otherwise create
    # the table without REFERENCES (app-level checks still apply).
    try:
        lakebase.run_write(
            f"""
